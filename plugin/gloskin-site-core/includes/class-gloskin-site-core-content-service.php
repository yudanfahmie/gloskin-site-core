...
final class Gloskin_Site_Core_Content_Service {
	...
	public static function register_content_types() {
		...
		register_post_type( self::PROMO_POST_TYPE, self::private_managed_post_type_args( __( 'Promos', 'gloskin-site-core' ), __( 'Promo', 'gloskin-site-core' ), array( 'title', 'thumbnail' ) ) );
		register_post_type( self::TESTIMONIAL_POST_TYPE, self::private_managed_post_type_args( __( 'Testimonials', 'gloskin-site-core' ), __( 'Testimonial', 'gloskin-site-core' ), array( 'title', 'excerpt', 'thumbnail' ) ) );
	 ...
	 */
	private static function private_managed_post_type_args( $plural, $singular, $supports = null ) {
		if ( null === $supports ) {
	...
	public static function register_taxonomies() {
		register_taxonomy( self::FAMILY_TAXONOMY, 'product', array(
			'labels' => array( 'name' => __( 'Jenis Produk', 'gloskin-site-core' ), 'singular_name' => __( 'Jenis Produk', 'gloskin-site-core' ) ),
			'public' => false, 'publicly_queryable' => false, 'show_ui' => false, 'show_in_menu' => false,
			'show_in_nav_menus' => false, 'show_in_rest' => false, 'show_admin_column' => false,
			'query_var' => false, 'rewrite' => false, 'hierarchical' => false,
		) );
		register_taxonomy( self::CONCERN_TAXONOMY, 'product', array(
			'labels' => array( 'name' => __( 'Keluhan', 'gloskin-site-core' ), 'singular_name' => __( 'Keluhan', 'gloskin-site-core' ) ),
			'public' => false, 'publicly_queryable' => false, 'show_ui' => false, 'show_in_menu' => false,
			'show_in_nav_menus' => false, 'show_in_rest' => false, 'show_admin_column' => false,
			'query_var' => false, 'rewrite' => false, 'hierarchical' => false,
		) );
		register_taxonomy( self::CONSULTATION_TAXONOMY, self::QUESTION_POST_TYPE, array(
			'labels' => array( 'name' => __( 'Jalur Konsultasi', 'gloskin-site-core' ), 'singular_name' => __( 'Jalur Konsultasi', 'gloskin-site-core' ) ),
			'public' => false, 'publicly_queryable' => false, 'show_ui' => false, 'show_in_menu' => false,
			'show_in_nav_menus' => false, 'show_in_rest' => false, 'show_admin_column' => false,
			'query_var' => false, 'rewrite' => false, 'hierarchical' => false,
		) );
	...
	private function register_question_answer_meta() {
		register_post_meta( self::QUESTION_POST_TYPE, self::ANSWER_META_KEY, array(
			'type' => 'array', 'single' => true, 'default' => array(),
			'show_in_rest' => array( 'schema' => array(
				'type' => 'array', 'items' => array( 'type' => 'object', 'properties' => array(
					'label' => array( 'type' => 'string' ), 'concern_id' => array( 'type' => 'integer' ), 'weight' => array( 'type' => 'integer' ),
				) ), 'default' => array(),
			) ),
	...
	public function sanitize_answer_options( $value ) {
		...
		foreach ( array_slice( $value, 0, self::ANSWER_MAX_OPTIONS ) as $option ) {
			...
			if ( ! $concern_id || ! term_exists( $concern_id, self::CONCERN_TAXONOMY ) ) { continue; }
			$weight = isset( $option['weight'] ) ? absint( $option['weight'] ) : 1;
			$weight = max( 1, min( self::ANSWER_MAX_WEIGHT, $weight ) );
			$clean[] = array( 'label' => $label, 'concern_id' => $concern_id, 'weight' => $weight );
		}
		return $clean;
	}
	...
	private function register_path_term_meta() {
		register_term_meta( self::CONSULTATION_TAXONOMY, self::PATH_META_ORDER, array(
			'type' => 'integer', 'single' => true, 'default' => 0, 'show_in_rest' => true,
			'sanitize_callback' => 'absint', 'auth_callback' => array( $this, 'authorize_term_meta' ),
		) );
		register_term_meta( self::CONSULTATION_TAXONOMY, self::PATH_META_IMAGE_ID, array(
			'type' => 'integer', 'single' => true, 'default' => 0, 'show_in_rest' => true,
			'sanitize_callback' => array( $this, 'sanitize_attachment_id' ), 'auth_callback' => array( $this, 'authorize_term_meta' ),
		) );
		register_term_meta( self::CONSULTATION_TAXONOMY, self::PATH_META_BASELINE, array(
			'type' => 'array', 'single' => true, 'default' => array(),
			'show_in_rest' => array( 'schema' => array( 'type' => 'array', 'items' => array( 'type' => 'integer' ), 'default' => array() ) ),
			'sanitize_callback' => array( $this, 'sanitize_baseline_concerns' ), 'auth_callback' => array( $this, 'authorize_term_meta' ),
		) );
	}
	...
	public function sanitize_baseline_concerns( $value ) {
		...
		foreach ( array_slice( $value, 0, self::ANSWER_MAX_OPTIONS ) as $candidate ) {
			$id = absint( $candidate );
			if ( $id && term_exists( $id, self::CONCERN_TAXONOMY ) ) { $ids[] = $id; }
		}
	...
	public static function ensure_family_terms() {
		if ( ! taxonomy_exists( self::FAMILY_TAXONOMY ) ) { return; }
		foreach ( array( self::FAMILY_SKINCARE => __( 'Skincare', 'gloskin-site-core' ), self::FAMILY_TREATMENT => __( 'Perawatan', 'gloskin-site-core' ) ) as $slug => $label ) {
			if ( ! term_exists( $slug, self::FAMILY_TAXONOMY ) ) { wp_insert_term( $label, self::FAMILY_TAXONOMY, array( 'slug' => $slug ) ); }
		}
	}
	...
	public static function clinic_definitions() {
		return array(
			'kebayoran-baru' => 'Kebayoran Baru', 'tebet' => 'Tebet', 'bekasi' => 'Bekasi', 'cibubur' => 'Cibubur', 'serpong' => 'Serpong',
			'surabaya' => 'Surabaya', 'banjarmasin' => 'Banjarmasin', 'balikpapan' => 'Balikpapan', 'denpasar' => 'Denpasar',
		);
	}
	...
	public static function skincare_definitions() {
		return array(
			'facial-wash' => 'Facial Wash', 'day-cream-sunscreen' => 'Day Cream / Sunscreen', 'toner' => 'Toner', 'serum' => 'Serum',
			'acne-care' => 'Acne Care', 'anti-aging' => 'Anti-Aging', 'brightening-pigmentation-care' => 'Brightening & Pigmentation Care',
		);
	}
	...
	private static function post_type_args( $plural, $singular, $rewrite_slug ) {
		return array(
			...
			'public' => true, 'show_ui' => true, 'show_in_menu' => self::ADMIN_MENU_SLUG, 'show_in_rest' => true, 'has_archive' => false,
			'rewrite' => array( 'slug' => $rewrite_slug, 'with_front' => false ),
			'supports' => array( 'title', 'editor', 'excerpt', 'thumbnail', 'custom-fields' ),
			'map_meta_cap' => true, 'delete_with_user' => false, 'publicly_queryable' => true,
		);
	}
	...
	private function sanitize_string( $value, $sanitizer ) {
		...
		switch ( $sanitizer ) {
			...
			case 'promo_type':
				...
				return in_array( $value, array( 'limited', 'regular' ), true ) ? $value : '';
			case 'text':
