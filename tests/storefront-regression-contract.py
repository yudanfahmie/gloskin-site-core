...
# Public Gloskin REST reads stay nonce-free guest projections. The core owns
# the shared helper for its long-standing reads; the Shop-specific controller
# uses the same GET/credentials contract without intercepting browser APIs.
...
# Shop has exactly one active catalog marker/controller. The older core
# controller remains in the shared bundle for compatibility but cannot bind
# this template, while the Shop controller owns full state/request sequencing.
...
# Wishlist zero-badge remains presentation-only.
...
# Preserve the documented unfiltered compatibility owner and bounded product
# hydration approach; filtered q/price logic is tested separately.
...
# The REST sanitizer regression stays fixed.
...
require(header_version == kernel_version == '0.7.118', 'production version must be synchronized at 0.7.118')
