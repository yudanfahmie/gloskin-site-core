...
<<<<<<< HEAD
require('thumbs' in carousel_body and 'thumbActive' in carousel_body,'initPromoCarousel() activate() must sync thumbnail is-active state');require('thumbs[ti]' in js or 'thumb.addEventListener' in js,'thumbnail buttons must have click event listeners wired in JS');require('data-gloskin-promo-prev' in helpers and 'data-gloskin-promo-next' in helpers,'existing prev/next controls must remain in carousel template');require('data-gloskin-promo-dot' in helpers,'existing dot controls must remain in carousel template');require('setInterval' not in carousel_body,'initPromoCarousel() must not introduce autoplay (no setInterval)');require('Version: 0.7.155' in plugin_h,'plugin header must be 0.7.155');require("const VERSION = '0.7.155';" in kernel,'Kernel VERSION must be 0.7.155')
if failures:
    for f in failures:print('FAIL:',f)
    sys.exit(1)
print('promo-thumbnail-contract.py: OK (0.7.155)')
=======
require('thumbs' in carousel_body and 'thumbActive' in carousel_body,'initPromoCarousel() activate() must sync thumbnail is-active state');require('thumbs[ti]' in js or 'thumb.addEventListener' in js,'thumbnail buttons must have click event listeners wired in JS');require('data-gloskin-promo-prev' in helpers and 'data-gloskin-promo-next' in helpers,'existing prev/next controls must remain in carousel template');require('data-gloskin-promo-dot' in helpers,'existing dot controls must remain in carousel template');require('setInterval' not in carousel_body,'initPromoCarousel() must not introduce autoplay (no setInterval)');require('Version: 0.7.156' in plugin_h,'plugin header must be 0.7.156');require("const VERSION = '0.7.156';" in kernel,'Kernel VERSION must be 0.7.156')
if failures:
    for f in failures:print('FAIL:',f)
    sys.exit(1)
print('promo-thumbnail-contract.py: OK (0.7.156)')
