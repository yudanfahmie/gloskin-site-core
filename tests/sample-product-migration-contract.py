...
req('manage_woocommerce' in admin and 'check_ajax_referer' in admin and 'wp_ajax_' in admin,'admin auth'); req('wp_ajax_nopriv' not in admin+importer+js and 'register_rest_route' not in admin+importer+js,'no public path'); req('enqueue_admin_migration' in asset and "const VERSION = '0.7.154'" in kernel,'asset/version')
