...
for p in ps:
    ...
    for v in p.get('variations',[]): req(v['status']=='draft' and v['source_id'].startswith(p['source_id']+':'),'variation identity/status'); ids.append(v['source_id']); skus.append(v['sku'])
...
req('manage_woocommerce' in admin and 'check_ajax_referer' in admin and 'wp_ajax_' in admin,'admin auth'); req('wp_ajax_nopriv' not in admin+importer+js and 'register_rest_route' not in admin+importer+js,'no public path'); req('enqueue_admin_migration' in asset and "const VERSION = '0.7.17'" in kernel,'asset/version')
...
req('Import Sample Products' in js and 'Resume Import' in js and "th.textContent = 'Simple'" in js and "textContent = 'Variable'" in js and "th.textContent = 'Images'" in js,'one-click admin labels')
