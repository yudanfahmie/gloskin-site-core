...
if ( false === $migration || false === $admin || false === $kernel || false === $plugin_h ) {
	fwrite( STDERR, "Cannot read final migration contract sources\n" );
	exit( 1 );
}
...
ok( (bool) preg_match( "/const VERSION\s*=\s*'0\.7\.141'/", $kernel ), 'Kernel VERSION must be 0.7.141' );
ok( (bool) preg_match( '/^ \* Version: 0\.7\.141$/m', $plugin_h ), 'Plugin header Version must be 0.7.141' );
