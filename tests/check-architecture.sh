...
>>>>>>> 5a0d320f1b9d6d9c42825d5d362a7b5f45e89197
...
<<<<<<< HEAD
# AssetService is the canonical first-party asset owner. The exact Contact
# bootstrap trait has one pre-existing route-independent reCAPTCHA enqueue hook
# in the handoff baseline; keep that exception file-scoped so no third owner
# can appear unnoticed.
...
echo "$asset_owner_files" | grep -qF "$asset_service_file" || { echo "canonical AssetService no longer owns frontend asset enqueue" >&2; exit 1; }
unexpected_asset_owners="$(echo "$asset_owner_files" | grep -vF "$asset_service_file" | grep -vF "$contact_bootstrap_file" || true)"
...
[[ "$asset_owner_count" -le 2 ]] || { echo "expected at most AssetService + documented Contact reCAPTCHA owner, found $asset_owner_count" >&2; exit 1; }
if echo "$asset_owner_files" | grep -qF "$contact_bootstrap_file"; then
