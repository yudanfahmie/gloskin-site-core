...
// Treatment Finder path-media presentation: full-bleed rectangular card media,
...
expect(frontendCss.includes('min-height:clamp(270px,31vw,390px)'), 'Desktop path cards must scale via the clamp min-height');
expect(/\.gloskin-ui1-consultation__path-media\{[\s\S]*?overflow:hidden;[\s\S]*?width:100%;[\s\S]*?height:100%;[\s\S]*?\}/.test(frontendCss), 'Path media must fill the card and clip its content via overflow:hidden');
...
expect(frontendCss.includes('.gloskin-ui1-consultation__path-media>.gloskin-ui1-media{') && /\.gloskin-ui1-consultation__path-image,\n\.gloskin-ui1-consultation__path-media>\.gloskin-ui1-media\{[\s\S]*?display:block;[\s\S]*?overflow:hidden;[\s\S]*?width:100%;[\s\S]*?height:100%;[\s\S]*?\}/.test(frontendCss), 'Fallback media root must fill and remain clipped inside the card');
...
expect(frontendCss.includes('@keyframes gloskin-consultation-path-breathe{') && frontendCss.includes('50%{transform:scale(1.025)}'), 'Path media must use restrained compositor-only breathing keyframes');
expect(frontendCss.includes('animation:gloskin-consultation-path-breathe 6.4s ease-in-out infinite'), 'Path breath animation must use a single shared 6.4s timing base');
...
expect(/\.gloskin-ui1-consultation__path\.is-active\{[\s\S]*?border-color:var\(--gloskin-accent\);[\s\S]*?box-shadow:0 0 0 4px var\(--gloskin-accent-soft\)/.test(frontendCss), 'Selected path must retain accent border and accent-soft halo');
...
expect(/@media \(max-width:900px\)\{[\s\S]*?\.gloskin-ui1-consultation__paths\{grid-template-columns:repeat\(2,minmax\(0,1fr\)\)\}/.test(frontendCss), '720-900px Treatment paths must collapse to two columns');
expect(/@media \(max-width:720px\)\{[\s\S]*?\.gloskin-ui1-consultation__path\{min-height:290px\}/.test(frontendCss), 'At <=720px paths must have a reduced min-height');
expect(/@media \(max-width:480px\)\{[\s\S]*?\.gloskin-ui1-consultation__paths\{grid-template-columns:1fr\}/.test(frontendCss), 'At <=480px paths must collapse to one column');
expect(frontendCss.includes('@media (max-width:360px){') && frontendCss.includes('.gloskin-ui1-consultation__panel{padding-inline:14px}'), '320px source guard must preserve panel padding for narrow viewports');
