...
(function () {
  ...
  function wireVariationForm(form) {
    if (form.dataset.fixtureVariationReady === '1') { return; }
    ...
    const qty = quantity.querySelector('input.qty');
    ...
    function reset() {
      ...
      qty.min = '1'; qty.max = '5'; qty.step = '1'; qty.value = '1';
    ...
    function refresh() {
      ...
      const rose = finish.querySelector('option[value="rose"]');
      ...
      qty.min = String(match.min_qty); qty.max = String(match.max_qty); qty.value = '1';
      quantity.style.display = match.is_sold_individually === 'yes' ? 'none' : '';
...
with sync_playwright() as p:
    ...
    # Native Woo controls remain singular/canonical, but selects are no longer visual.
    ...
    # Incomplete click: zero mutation, reusable toast + one gentle attention sound, focus unresolved group.
    proxy.click()
    ...
    # Chip -> SAME select -> Woo variation state. No custom resolver in Gloskin.
    ...
    # Valid proxy click reaches the same native button exactly once and one AJAX mutation.
    proxy.click()
    ...
    # Fresh reopen from cache, sold-individually + disabled-option synchronization.
    open_quickadd(page)
    ...
    require(page.locator('.woocommerce-variation-add-to-cart').get_attribute('class').find('is-quantity-hidden') >= 0, 'sold-individually row must switch to full CTA geometry')

    # Reset through the SAME hidden native select and prove chips synchronize back.
    page.evaluate("""() => {
    ...
    # Transparent wrapper + practical mobile widths: no secondary card/overflow.
    wrapper = page.locator('.table-container')
