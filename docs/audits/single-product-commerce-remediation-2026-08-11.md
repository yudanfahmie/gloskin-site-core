...
## 2026-08-12 follow-up: dock full-container-width accent surface + entrance animation

Direct owner-requested visual follow-up on the same dock, not a new defect finding. `gloskin-ui1-purchase-dock.js`'s `updateGeometry()` now sizes/positions the floating and settled dock from `product.getBoundingClientRect()` (the whole two-column product grid) instead of `.summary`'s own rect, so the dock is a full-container-width bar in both states rather than matching only the summary column -- one rect is now the single source of truth for both states instead of two. `gloskin-ui1-core.css`'s `.gloskin-ui1-purchase-dock` surface changed from `var(--gloskin-bg)` to the Gloskin accent red (`var(--gloskin-accent)`/`var(--gloskin-inverse)` text), with a scoped contrast inversion for `.single_add_to_cart_button` (inverse background/accent text, so it never disappears against its own accent-red backdrop) and `.stock` text -- same Form/Action Kit, one more scoped context, not a second button skin. `setState()` gained a transform-only entrance: entering the fixed floating state (never the in-flow settled/boundary state) sets `transform:translateY(100%)`, flushes layout, then clears it on the next animation frame so the CSS `transition:transform 260ms` on `.is-floating` animates it sliding up from the bottom -- compositor-only, no layout thrash, degrades to an instant snap under `prefers-reduced-motion` via the existing global transition-duration override. Zero new `!important`. Proven via `tests/purchase-dock-controller-contract.sh` (full-width geometry source, entrance transform lifecycle) and the extended `tests/single-product-dock-browser-smoke.py` (`productLeft`/`productWidth` geometry assertions, settled-transform assertion).
