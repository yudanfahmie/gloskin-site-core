...
The current Home presentation intentionally uses the existing shared hero/video owner in video-only mode: no visible eyebrow/heading/copy/CTA, one native background `<video>` sourced from a WordPress Media Library MP4 or WebM attachment (muted, autoplay, loop, playsinline, true `object-fit:cover`), and a pointerless media/loading layer. Its pure-white preparing state reconciles media that loaded or began playing before initialization; normal motion requires usable data plus both resolved `play()` and observed/reconciled playback. The safety timeout releases the loader without preventing later readiness recovery, native media error keeps the clean white fallback, and reduced motion reveals a paused static frame. Bottom gradient and one SVG scroll cue to the actual following section remain. No YouTube network/runtime, iframe, poster, player, or Play chrome exists on Home; do not create a second Home hero/video service.
