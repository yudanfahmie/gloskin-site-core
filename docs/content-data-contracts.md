...
The optional synthetic demo importer is an explicit privileged admin action. It is available without deployment-environment configuration and is protected by the `manage_woocommerce` capability, nonce verification, explicit synthetic-data confirmation, deterministic identities, idempotent/resumable behavior, collision guards and one-shot consumed state. Its target is at least four paths, ten concerns, thirteen questions and exactly eight Woo Treatment Products; demo products remain distinct from the existing eight informational `gloskin_treatment` records. This bundle is synthetic demonstration data, not verified commercial or clinical truth.
...
The current Home presentation intentionally uses the existing shared hero/video owner in strict video-only mode: no visible eyebrow/heading/copy/CTA and one native background `<video>` sourced only from the `hero_video_media_id` Media Library MP4/WebM attachment (muted, autoplay, loop, playsinline, `preload="auto"`, true `object-fit:cover`). Its pure-white preparing state and pointerless loader reconcile media that loaded or began playing before initialization; normal motion requires usable data plus both resolved `play()` and observed/reconciled playback before the roughly 360ms fade-in. The safety timeout releases the loader without preventing later readiness recovery, native media error keeps the clean white failure state, and reduced motion reveals a paused native frame. Missing, deleted, URL-less or unsupported attachments produce a clean white unavailable state with no indefinite loader. Featured images and editorial media are explicitly excluded from this Home branch. Bottom gradient and one SVG scroll cue to the actual following section remain. No YouTube network/runtime, iframe, poster, player or Play chrome exists on Home; do not create a second Home hero/video service or option.
