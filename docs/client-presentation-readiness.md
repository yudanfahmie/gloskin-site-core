...
| 8. Media quality | DONE | Public templates use WordPress attachment rendering rather than hand-built `<img>` URLs. Missing media omits the media component; hero media requests a responsive WordPress size instead of the original full-size upload. |
...
| 12. Client presentation QA | DONE in available browser | Browser smoke covers 13 representative views at 390x844, 820x1180 and 1440x900. It checks overflow, heading structure, client-facing leak strings and console/page errors. |
...
Automated browser presentation checks currently run in Chromium when available. Safari/WebKit and Firefox remain launch-environment checks and must not be claimed unless they are actually run in an environment that provides those browsers.
