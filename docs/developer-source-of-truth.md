...
Current production Home intentionally specializes the existing shared hero renderer into `video-only` mode. It keeps the same admin-configurable hero-video owner and must render no visible eyebrow/heading/copy/CTA; a screen-reader structural heading is acceptable. The media is a native `<video>` sourced from a WordPress Media Library attachment (`hero_video_media_id` in the existing `gloskin_site_core_settings` option) -- muted, autoplay, loop, playsinline, preloaded, no controls/poster/Play chrome, no iframe -- true full-bleed `object-fit:cover` (never letterboxed), starting from a pure-white preparing state with a small non-interactive loader until real playback readiness (loadeddata/playing + a resolved `play()` Promise) is proven, then fading in. A clean white failure state releases the loader if the video never becomes usable. When no native video is configured, the same media fallback chain (attachment image -> editorial placeholder) takes over. Bottom transparent-to-surface gradient, exactly one SVG scroll cue targeting the actual next sibling section, and reduced-motion behavior (established/paused static frame, no repeated loader motion) are unchanged. The legacy YouTube-nocookie poster/facade/iframe enhancement remains available only for the `standard` hero mode's backward compatibility; Home's `video-only` mode never renders it. Do not build a second Home hero/video service.
