...
Current production Home intentionally specializes the existing shared hero renderer into strict `video-only` mode. It keeps one admin-configurable owner and must render no visible eyebrow/heading/copy/CTA; a screen-reader structural heading is acceptable. Its only visible media is one native `<video>` sourced from a WordPress Media Library MP4 or WebM attachment (`hero_video_media_id` in the existing `gloskin_site_core_settings` option) -- muted, autoplay, loop, playsinline, `preload="auto"`, no controls/poster/Play chrome, no iframe -- with true full-bleed `object-fit:cover` (never letterboxed) and a pointerless media/loading layer. It starts on a pure-white preparing surface; the video remains invisible until real playback readiness and then fades in over roughly 360ms. The controller installs media listeners before reconciling an already-usable `readyState` or already-playing state; normal-motion readiness requires usable data, a resolved `play()` Promise, and observed/reconciled playback. The bounded timeout only releases the loader into a clean white failure state and remains recoverable if valid readiness arrives later; a native media error is terminal. Reduced motion pauses and reveals a usable native frame whether data was ready before or after initialization. When no supported native source is configured/resolvable, Home remains a clean white hero with no loader; it never falls back to featured, attachment or editorial imagery. Bottom gradient and exactly one SVG scroll cue targeting the actual next sibling section remain. There is no Home YouTube network/runtime path and no second Home hero/video service.
